...
def parse_args():
    ...
    parser.add_argument(
        '--show_dir', type=str, default='', help='directory where results will be saved')
    parser.add_argument(
        '--gpu-collect',
        action='store_true',
...
def main():
    ...
    occupancy_results_path = "./val/result/occupancy_results.pkl"
    flow_results_path = "./val/result/flow_results.pkl"
    have_results = False 

    if osp.isfile(occupancy_results_path) and osp.isfile(flow_results_path):
        occupancy_results = mmcv.load(occupancy_results_path)
        flow_results = mmcv.load(flow_results_path)
        have_results = True
    else:
    ...
    for index, occ_threshold in enumerate(occ_thresholds):
        if not have_results:
            ...
            mmcv.dump(occupancy_results, occupancy_results_path)
            mmcv.dump(flow_results, flow_results_path)
