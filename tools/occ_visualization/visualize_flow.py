...
def get_point_color(points, labels):
    x = points[:, 0]  # x position of point
    y = points[:, 1]  # y position of point
    z = points[:, 2]  # z position of point
...
def indice2index(indice,occ_xdim=200):
    x = indice % occ_xdim
    y = (indice // occ_xdim) % occ_xdim
    z = indice // (occ_xdim*occ_xdim)
    return x,y,z

def get_flwgrid(occdata,flowdata):
    occ_index = occdata[:, 0]
    flow_grid=np.zeros((200,200,16,3))
    flow_data_extend = np.concatenate((flowdata, np.zeros_like(flowdata[:,0])[:, np.newaxis]), axis=-1)
    for id,indice in  enumerate(occ_index):
        x,y,z=indice2index(indice)
        flow_grid[x,y,z]=flow_data_extend[id]
    # flow_grid=torch.tensor(flow_grid)
    return flow_grid
...
if __name__ == '__main__':
    occ_path = '/mnt/sata4t/dataset/occ_gt_release_v1_0/train/scene-0001/000_occ.npy'  # TODO set occupancy path
    flow_path = '/mnt/sata4t/dataset/occ_gt_release_v1_0/train/scene-0001/000_flow.npy'  # TODO set occupancy path
    ...
    u, v = flow[..., 0], flow[..., 1] # 提取矢量分量
    w = np.zeros_like(u)
    colors= get_point_color(points, labels)
    # colors=
    # 使用quiver3d函数进行可视化
    # visualize_occ(points, labels, ego_dict)
    
    plot=mlab.quiver3d(x, y, z, u, v, w ,scalars=colors,scale_mode = "vector",colormap='rainbow')
